...
wifi_settings wifi_adapter::ws;
int wifi_adapter::tries = 0;

void wifi_adapter::init(wifi_settings a_ws){
...
esp_err_t wifi_adapter::event_handler(void *ctx, system_event_t *event){
...
void wifi_adapter::eth_gpio_config_rmii(){
...
void wifi_adapter::start(){
    ...
    const char* ssid = ws.get_ssid().c_str();
    const char* pass = ws.get_pass().c_str();
